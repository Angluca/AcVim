...
hi def link cppStructure		cppTypedef
...
syn match Operator       '[\+\-\%=\^\&\*!?><\$|]'
syn match cpponstant       '[{}\[\]()]'

hi def link cppKeyword Keyword
hi def link cppFunction Function
hi def link cppTypedef Identifier
hi def cppSymbol ctermfg=DarkGray guifg=DarkGray
hi def cppType ctermfg=DarkCyan guifg=DarkCyan
syn match cppTypedef  contains=cppTypedef "\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*" display contained
syn match cppFunction    "\%(r#\)\=\%([^[:cntrl:][:space:][:punct:][:digit:]]\|_\)\%([^[:cntrl:][:punct:][:space:]]\|_\)*" display contained
syn keyword cppKeyword union struct enum namespace nextgroup=cppTypedef skipwhite skipempty
syn keyword cppKeyword enum nextgroup=cppType skipwhite skipempty contained
syn match Repeat        "\([^\.]\.\)\@<=\w\w*\(\(\[.*\]\)*\_s*(\)\@!"
syn match cppFunction   "[0-9a-zA-Z_@]*\w\w*\(\(\[.*\]\)*\_s*(\)\@="
"syn match cppType       '(\=\_s*\(\[.*\]\_s*\)*\(const\_s*\)\=\zs\w\w*\ze\(\[.*\]\)*\_s*\(\[.*\]\)*\_s*{'
